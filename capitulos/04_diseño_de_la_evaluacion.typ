...
-	*Relevancias conocidas (_qrels_)*: Los juicios de relevancia son esenciales para evaluar el desempeño de los métodos de manera objetiva, al incluir datasets con qrels bien establecidos, se garantiza que los resultados estén basados en un marco estandarizado, facilitando su interpretación y comparación
...
Un criterio fundamental en la selección fue la preferencia por datasets con juicios de relevancia multinivel (_graded relevance judgments_). Esta decisión se fundamenta en las siguientes ventajas:
...
- *Importancia histórica*: Los experimentos de Cranfield establecieron el "paradigma Cranfield" que incluye una colección de documentos, un conjunto de consultas con juicios de relevancia asociados, y evaluación basada en precisión y _recall_, metodología que posteriormente adoptaría TREC.
\
...
- *Consultas*: El conjunto "_test_" incluye 200 preguntas no factuales de dominio abierto, caracterizadas por ser subjetivas y basadas en opiniones, donde no existe una única respuesta "correcta". Las preguntas provienen de categorías diversas como consejos, recomendaciones y experiencias personales.
- *Origen de Qrels*: Los 6,589 juicios de relevancia del conjunto "_test_" fueron recolectados mediante crowdsourcing en Amazon Mechanical Turk. Los anotadores evaluaron todas las respuestas disponibles para cada pregunta utilizando una escala de 4 niveles: nivel 1 (fuera de contexto), nivel 2 (no relevante), nivel 3 (marginalmente relevante), y nivel 4 (altamente relevante). La distribución incluye: 1.642 juicios de nivel 1, 2.417 de nivel 2, 1.196 de nivel 3, y 1.334 de nivel 4.
...
En la @tbl:tabla-metricas-datasets se detalla la configuración específica de relevancia, umbrales binarios y valores de ganancia para cada dataset seleccionado.
