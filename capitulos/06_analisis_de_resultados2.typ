...
4.  *Coeficiente de Referencia ($τ$)*: Finalmente, cada diagrama incorpora el valor de correlación de Kendall (τ) calculado para ese par específico de variables. La inclusión de esta métrica permite contrastar la percepción visual de la dispersión con la medida estadística formal de asociación ordinal, lo que es fundamental para contextualizar la línea de regresión, ya que mientras la línea muestra la tendencia lineal general, el valor τ cuantifica la calidad del ranking.
...
Los métodos post-retrieval basados en divergencia (NQC, WIG y UEF-NQC) dominan visualmente el escenario, presentando las pendientes positivas más pronunciadas y, crucialmente UEF-NQC exhibe la banda de confianza más estrecha y una nube de puntos más compacta que su versión base (NQC), lo que confirma visualmente que la reestimación de utilidad reduce el ruido de la predicción.

En contraste, los métodos pre-retrieval (IDF promedio y máximo) muestran líneas de tendencia horizontales con una dispersión vertical extrema. Es notable cómo consultas con el máximo puntaje IDF pueden tener un nDCG de 0, evidenciando la desconexión entre la rareza de los términos y su relevancia real en este dataset. SCQ muestra un patrón distintivo en su variante #emph[SCQ-Max]: este método selecciona como puntaje el término más discriminativo de la consulta, lo que explica la agrupación observada en el rango $x in {45 dash 55}$. Esto sugiere que la mayoría de las consultas contienen al menos un término medianamente raro que domina el puntaje final, provocando que el método pierda sensibilidad para distinguir entre consultas fáciles y difíciles.
...
En #emph[TREC-COVID] (@fig:dispersion_covid) emerge un patrón distintivo que invierte las jerarquías observadas en los otros conjuntos de datos, puesto que visualmente Clarity y su variante UEF-Clarity dominan el espectro, presentando las pendientes positivas más pronunciadas y bandas de confianza estrechas, lo que denota una predicción robusta y bien correlacionada.

Por el contrario, los métodos basados en divergencia de puntajes (NQC, WIG), habitualmente dominantes en otros conjuntos de datos, colapsan en líneas de tendencia casi horizontales. Esta inversión se atribuye a la naturaleza técnica y homogénea del corpus:
...
- En _TREC-COVID_ resurge como uno de los métodos líderes (τ = 0,320), y más aún, su variante _UEF-Clarity_ alcanza en este dataset el valor más alto de la tabla (τ = 0,331).

Esto indica que los modelos de lenguaje, base para _Clarity_, son altamente efectivos cuando el corpus posee una homogeneidad temática y terminológica (como son los artículos médicos sobre COVID-19), pero se vuelven inestables e inútiles en colecciones pequeñas o con vocabularios más dispersos.
...
En este dataset, las correlaciones de NQC y UEF-NQC son idénticas (τ = 0,408), lo que sugiere que, en corpus de gran escala y diversidad temática, la expansión de consultas, que es el mecanismo central de _UEF_, podría no estar aportando información nueva relevante o, peor aún, podría estar introducción ruido que neutraliza las ganancias de la re-estimación.
...
En consecuencia, se establece que métodos como _UEF-NQC_ constituyen un _baseline_ experimental idóneo para futuras investigaciones, debiendo preferirse sobre métodos más débiles como _IDF_, siempre que el costo computacional de la expansión de consultas sea admisible.
...
Es fundamental también reconocer las limitaciones inherentes al estándar de referencia (_Ground Truth_) utilizado para validar estas predicciones, ya que métricas de recuperación como _nDCG_ dependen enteramente de la completitud y calidad de los juicios de relevancia humanos, por lo que, en datasets con escasa profundidad de juicio (_sparse judgments_) o sesgos de anotación, lo que los métodos QPP intentan predecir no es necesariamente la satisfacción real del usuario, sino la coincidencia con un conjunto de etiquetas estáticas, lo que puede ser particularmente crítico en casos como _CAR_, donde la baja correlación podría estar reflejando no solo la incapacidad de los predictores, sino también la desconexión entre la definición teórica de relevancia del dataset y la utilidad real percibida que los métodos estadísticos intentan inferir.
