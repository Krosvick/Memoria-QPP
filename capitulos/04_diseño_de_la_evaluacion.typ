...
En cuanto al análisis de las fortalezas y debilidades de cada método seleccionado:
...
-	Clarity Score (SC): Mide la coherencia del lenguaje de los documentos recuperados, lo que lo hace efectivo en consutlas específicas con temas bien definidos, pero es sensible a consultas cortas o ambiguas, donde la divergencia entre el modelo de lenguaje y el corpus puede ser menos clara.
...
-	MS MARCO (Passage): Proporciona un entorno realista donde las consultas derivan de necesidades cotidianas de usuarios reales, sus relevancias asignadas por evaluadores humanos lo convierten en un recurso clave para probar la aplicabilidad práctica de los métodos QPP. Evalúa la capacidad de los métodos para manejar consultas informacionales y transaccionales en contextos cotidianos.
-	Antique/Test: Este dataset introduce complejidad al centrarse en preguntas y respuestas subjetivas basadas en opiniones, lo que plantea un desafío adicional para los métodos QPP al modelar consultas no factuales. Analiza cómo los métodos manejan preguntas subjetivas y abiertas, evaluando su capacidad para predecir el rendimiento en dominios con alta variabilidad semántica.
-	BEIR-TREC-COVID: Su enfoque en consultas científicas relacionadas con COVID-19 lo hace altamente relevante para escenarios especializados donde la precisión y la especificidad son esenciales. Evalúa el rendimiento de los métodos en un dominio crítico donde la información relevante es escasa y de alta relevancia.
-	CAR (Complex Answer Retrieval): Al estar enfocado en consultas complejas derivadas de la estructura de Wikipedia, este dataset permite analizar cómo los métodos QPP manejan escenarios con alta complejidad semántica. Mide la capacidad de los métodos para predecir el rendimiento en consultas que requieren una interpretración profunda y modelado avanzado.
...
\
El diseño experimental sigue un flujo bien definido para garantizar la reproducibilidad y el análisis riguroso de los resultados. El flujo incluye los siguientes pasos principales:

\
-	*Preparación de Datasets:* Los datasets seleccionados, como Cranfield o MS MARCO, se descargan utilizando la librería ir_datasets y se preprocesan en el entorno Docker, este proceso incluye la extracción de consultas, preparación de los juicios de relevancia (qrels) y el formateo adecuado para su uso con PyTerrier.
-	*Indexado de Datasets:* Los datasets se indexan utilizando PyTerrier, creando estructuras de datos eficientes para la recuperación de información. Es también en este paso donde se guardan metadatos de los datasets, como la frecuencia de los términos en los documentos como la frecuencia de estos en la colección.
...
-	*Evaluación utilizando juicios de relevancia (qrels):* Se realiza una evaluación del rendimiento de las consultas utilizando la librería de ir_measures, esto es el "ground truth" o "verdadero valor" del rendimiento de la consulta en el sistema de recuperación implementado. Posteriormente los resultados se almacenan en formato estructurado para su posterior análisis.
...
El sistema utiliza el modelo de recuperación BM25 (Best Match 25) como método principal de recuperación. Los parámetros han sido cuidadosamente seleccionados basándose en la literatura y experimentación previa:
...
#figure(
  table(
    ...
    [--correlations], [Coeficientes de correlación a calcular], [τ],
    [--use-uef], [Habilita el método UEF (Utility Estimation Framework)], [False],
...
\
-	*Métricas Utilizadas*:
Las métricas de evaluación IR se deben configurar específicamente para cada dataset, considerando sus características particulares en los juicios de relevancia (qrels).
...
La implementación utiliza ir_measures para garantizar cálculos estandarizados de las métricas IR, mientras que scipy.stats proporciona implementaciones eficientes de los coeficientes de correlación. El sistema maneja automáticamente casos especiales como queries sin resultados o scores QPP indefinidos, asegurando una evaluación robusta incluso en condiciones no ideales.  
