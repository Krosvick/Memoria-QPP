...
\
El método SCQ (Similarity Between a Query and a Collection), es un predictor pre-retrieval propuesto por Ying Zhao, Falk Scholer y Yohannes Tsegay. En el artículo @preretrieval-idf los autores explican que el SCQ calcula un puntaje de similitud entre una consulta y la colección de documentos, utilizando la frecuencia de términos en la colección y la frecuencia inversa de documentos (IDF) como evidencias para determinar la relevancia. Siendo una de sus principales ventajas que se basa en estadísticas disponibles durante el proceso de indexado, eliminando la necesidad de realizar búsquedas previas.
...
\
En donde $q$ corresponde a la consulta, $M$ al modelo de recuperación, $D[k]_q$ a la lista de los k documentos mejor rankeados, μ al promedio de los puntajes de recuperación en $D[k]_q$ y $S c o r e[D]$ al puntaje de recuperación del corpus considerado como un único documento concatenado.
