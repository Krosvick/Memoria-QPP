...
El conjunto completo de documentos disponibles para ser buscados se denomina corpus o colección. Por otra parte, la consulta o #emph[query] corresponde a la expresión, generalmente breve, con la que el usuario comunica su necesidad de información, esta puede estar compuesta por palabras clave, frases o descripciones más amplias.

En los sistemas de evaluación, estas consultas se agrupan junto con sus documentos y juicios de relevancia en lo que se conoce como colecciones de referencia o #emph[query logs], las cuales permiten medir objetivamente el rendimiento de distintos modelos de recuperación.
...
A modo de síntesis, la figura @fig:ir-components-diagram ilustra cómo estos procesos se articulan dentro de un sistema de Recuperación de Información, mostrando el flujo que existe desde la colección de documentos y la consulta del usuario hasta la obtención del ranking final y su posterior evaluación.
...
El cálculo de estos pesos sigue criterios estadísticos, en donde la técnica estándar es el #emph[esquema TF-IDF], que combina dos componentes: #emph[la frecuencia del término (TF)] que mide cuántas veces aparece un término dentro de un documento y #emph[la frecuencia inversa de documento (IDF)] que indica cuán raro o discriminativo es el término dentro del corpus completo. De esta forma, se otorga mayor importancia a palabras que son relevantes dentro de un documento pero poco frecuentes en la colección, mejorando la capacidad del sistema para distinguir documentos pertinentes @query-difficulty-book.
...
#v(10pt)
1. *Percentiles:* Particiona la distribución de la métrica $M$ por consulta mediante percentiles. En el caso binario, una consulta $q$ se considera *difícil* para un sistema $S_i$ si su valor $m_S_i (q)$ es menor o igual que un percentil $p_x$, es decir, si aproximadamente $x\%$ de las consultas tienen un valor inferior. En el caso de desear una *dificultad graduada*, se utilizan $N-1$ percentiles para definir $N$ clases ordenadas de dificultad, donde la primera clase agrupa las consultas más difíciles y la última las más fáciles.#block(width: 100%, $ p_x : "dificultad"(q) = 1_{m_s_i (q) <= p_x} $)En estas expresiones, $"dificultad"(q)$ es una función *indicadora* que toma el valor 1 cuando la consulta $q$ se etiqueta como *difícil* y 0 en caso contrario. El parámetro $p_x$ corresponde al percentil $x$‑ésimo de la distribución de $M$ sobre todas las consultas, de modo que $1_{m_{S_i}(q) <= p_x}$ marca como *difíciles* aquellas cuyo rendimiento cae en el $x\%$ inferior de la distribución.
#block(width: 100%, $
  q in cases(
...
$)
  En el caso graduado, los símbolos $D_1, ..., D_N$ representan *clases ordenadas de dificultad* (por ejemplo, *muy difícil*, *difícil*, *fácil*, *muy fácil*), mientras que $P_1, ..., P_{N-1}$ son percentiles crecientes que dividen la distribución de $M$ en $N$ segmentos. Cada consulta $q$ se asigna a una clase $D_k$ en función del intervalo en el que cae su valor $m_{S_i}(q)$ con respecto a estos cortes percentilares.    
2. *Umbrales:* Fija cortes absolutos sobre la métrica para definir clases de dificultad. En el caso binario, una consulta $q$ es *difícil* para $S_i$ si $m_S_i (q)$ es menor o igual que un umbral $T$ (por ejemplo, consultas con $P@10 <= 0.1$). Para el caso graduado, se emplea un conjunto de umbrales $T_1, ..., T_(N-1)$ que inducen $N$ clases de dificultad ordenadas. Existen dos configuraciones típicas:
  #v(4pt)
   #block(width: 100%, $ T : "dificultad"(q) = 1_{m_s_i \\(q) <= T} $) En la definición por umbrales, $T$ es un *corte absoluto* fijado directamente sobre la escala de la métrica $M$. De nuevo, $"dificultad"(q)$ es una función indicadora que vale 1 si el rendimiento $m_{S_i}(q)$ de la consulta cae por debajo o en el propio umbral $T$ (es decir, si la consulta no alcanza un mínimo de efectividad considerado aceptable) y 0 en caso contrario. 
...
3. *Combinada:* Combina ambos criterios: percentiles y por umbral. En el caso binario, una consulta se considera *difícil* si es clasificada como difícil tanto por la definición basada en percentiles como por la basada en umbrales (es decir, si $m_S_i (q) <= T$ y $m_S_i (q) <= p_x$). Para dificultad graduada, la clase asignada a una consulta corresponde a la intersección de las clases obtenidas con cada una de las dos estrategias anteriores.#block(width: 100%, $
...
#v(4pt)
Desde el punto de vista práctico, esta definición restringe la etiqueta de dificultad a consultas que son claramente *mal resueltas* tanto en términos relativos (respecto del resto de consultas, vía percentil) como absolutos (respecto de un nivel mínimo de efectividad, vía umbral). Esto tiende a producir conjuntos de entrenamiento más contrastados para los modelos de predicción de dificultad, centrados en los casos realmente extremos.
...
Para la evaluación de _QPP_ se utiliza un protocolo experimental que utiliza las métricas de efectividad calculadas por consulta (por ejemplo, _AP_, _MAP_, _nDCG_) correlacionandolas directamente con el valor del predictor para la misma consulta. La comparación emplea *coeficientes de correlación de rangos* y se acompaña de *pruebas de significancia e intervalos de confianza* para estimar la robustez. De este modo, la utilidad del predictor se juzga por la magnitud y la estabilidad de las correlaciones y por su capacidad para guiar decisiones.
